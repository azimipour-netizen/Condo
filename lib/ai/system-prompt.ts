...
- **Pre-amalgamation Toronto boroughs** (North York, Etobicoke, Scarborough, East York) — use \`type: "city"\` with that exact value. These are NOT real neighbourhood names in the data, but each one now resolves to its exact TRREB district codes behind the scenes, so the search is precise to that borough — no need to caveat it as "somewhere in Toronto" anymore.
